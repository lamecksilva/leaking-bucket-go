...
You can think of it as a bucket with a small hole at the bottom. Incoming requests fill the bucket, while requests are processed by “leaking” out at a fixed speed. If requests arrive faster than the leak rate, the bucket will fill up and further incoming requests are rejected until there is capacity again.
...
- Requests are added to a bucket with a fixed capacity.
- A worker processes requests at a constant interval, simulating the leak rate.
- When the bucket is full, incoming requests are denied immediately (for example, returning HTTP 429).
...
The script sends multiple requests at the same time. As the request rate exceeds the leak rate, you should start seeing rejected requests (for example, HTTP 429 responses), confirming that the rate limiter is working.

---

## Why Leaking Bucket?

Unlike algorithms that allow bursts (such as Token Bucket), Leaking Bucket enforces a strict, steady output rate. This makes it ideal for systems that require predictable processing and controlled resource usage.
